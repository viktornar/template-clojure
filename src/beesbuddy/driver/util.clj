(ns beesbuddy.driver.util
  ...
  (:require
   ...
   [beesbuddy.public-settings.premium-features :as premium-features]
   [beesbuddy.query-processor.error-type :as qp.error-type]
   [beesbuddy.query-processor.store :as qp.store]
   [beesbuddy.util :as u]
   [beesbuddy.util.i18n :refer [deferred-tru trs]]
...
(def ^:private connection-error-messages
  ...
  of [[beesbuddy.driver/humanize-connection-error-message]]."
...
(defsetting db-connection-timeout-ms
  "Consider [[beesbuddy.driver/can-connect?]] / [[can-connect-with-details?]] to have failed if they were not able to
...
(defn available-drivers
  ...
  []
  (set (for [driver (descendants driver/hierarchy :beesbuddy.driver/driver)
  ...
  (let [prop-name (:name prop)
    ...
    [{:name type-prop-nm
     ...
     {:name (str prop-name "-patterns")
      ...
      :description (trs "Comma separated names of {0} that should appear in beesbuddy" (u/lower-case-en disp-name))
     ...
     {:name (str prop-name "-patterns")
      :type "text"
      :placeholder "E.x. public,auth*"
      :description (trs "Comma separated names of {0} that should NOT appear in beesbuddy" (u/lower-case-en disp-name))
