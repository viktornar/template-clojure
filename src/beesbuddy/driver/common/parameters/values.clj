(ns beesbuddy.driver.common.parameters.values
  ...
  (:require
   ...
   [beesbuddy.driver.common.parameters :as params]
   [beesbuddy.legacy-mbql.schema :as mbql.s]
   [beesbuddy.lib.metadata :as lib.metadata]
   [beesbuddy.lib.metadata.protocols :as lib.metadata.protocols]
   [beesbuddy.lib.schema.common :as lib.schema.common]
   [beesbuddy.lib.schema.id :as lib.schema.id]
   [beesbuddy.lib.schema.template-tag :as lib.schema.template-tag]
   [beesbuddy.models.native-query-snippet :refer [NativeQuerySnippet]]
   [beesbuddy.query-processor.compile :as qp.compile]
   ...
   [beesbuddy.query-processor.util.persisted-cache :as qp.persistence]
   [beesbuddy.util :as u]
...
(def ^:private Date                   (lib.schema.common/instance-of-class beesbuddy.driver.common.parameters.Date))
(def ^:private FieldFilter            (lib.schema.common/instance-of-class beesbuddy.driver.common.parameters.FieldFilter))
(def ^:private ReferencedQuerySnippet (lib.schema.common/instance-of-class beesbuddy.driver.common.parameters.ReferencedQuerySnippet))
(def ^:private ReferencedCardQuery    (lib.schema.common/instance-of-class beesbuddy.driver.common.parameters.ReferencedCardQuery))

(defmulti ^:private parse-tag
  "Parse a tag by its `:type`, returning an appropriate record type such as
  `beesbuddy.driver.common.parameters.FieldFilter`."
  {:arglists '([tag params])}
  (fn [{tag-type :type} _]
    (keyword tag-type)))
