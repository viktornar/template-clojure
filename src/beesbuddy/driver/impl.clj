(ns beesbuddy.driver.impl
  "Internal implementation functions for [[beesbuddy.driver]]. These functions live in a separate namespace to reduce the
  clutter in [[beesbuddy.driver]] itself."
  (:require
   [beesbuddy.lib.util :as lib.util]
   [beesbuddy.plugins.classloader :as classloader]
   [beesbuddy.util :as u]
   [beesbuddy.util.i18n :refer [trs tru]]
...
(defn registered?
  ...
  (with-load-driver-read-lock
    (isa? hierarchy (keyword driver) :beesbuddy.driver/driver)))
...
(mu/defn ^:private driver->expected-namespace [driver :- :keyword]
  (symbol
   (or (namespace driver)
       (str "beesbuddy.driver." (name driver)))))
...
(defn load-driver-namespace-if-needed!
  "Load the expected namespace for a `driver` if it has not already been registed. This only works for core beesbuddy
...
(defn register!
  ...
  You can add additional parents to a driver using [[beesbuddy.driver/add-parent!]]; this is how test extensions are
  implemented.
  ...
  (when-not *compile-files*
    (let [parents (filter some? (u/one-or-many parent))]
      ...
      ;; ok, if that was successful we can derive the driver from `:beesbuddy.driver/driver`/`::concrete` and parent(s)
      (let [derive! (partial alter-var-root #'hierarchy derive driver)]
        (derive! :beesbuddy.driver/driver)
        (when-not abstract?
          (derive! ::concrete))
        (doseq [parent parents]
          (derive! parent)))
      ...
      (log/info
       (u/format-color 'blue
                       (format (if (beesbuddy.driver.impl/abstract? driver)
...
(defonce ^:private initialized-drivers
  ...
  (atom #{:beesbuddy.driver/driver ::concrete}))

(defn initialized?
  "Has `driver` been initialized? (See [[beesbuddy.driver/initialize!]] for a discussion of what exactly this means.)"
  [driver]
...
(defn initialize-if-needed!
  ...
  for [[beesbuddy.driver/initialize!]] for a full explanation of what this means."
  [driver init-fn]
  ;; no-op during compilation
  (when-not *compile-files*
...
(def default-alias-max-length-bytes
  ...
  of [[beesbuddy.driver/escape-alias]]."
