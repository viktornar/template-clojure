(ns beesbuddy.driver.common
  ...
  (:require
   ...
   [beesbuddy.util.i18n :refer [deferred-tru]]
...
(def let-user-control-scheduling
  ...
  {:name         "let-user-control-scheduling"
   ...
   :description  (deferred-tru "By default, beesbuddy does a lightweight hourly sync and an intensive daily scan of field values. If you have a large database, turn this on to make changes.")
   :visible-if   {"advanced-options" true}})
...
(def cache-field-values-schedule
  ...
  {:name "schedules.cache_field_values"
   ...
   :description  (deferred-tru
                   (str "beesbuddy can scan the values present in each field in this database to enable checkbox "
                        "filters in dashboards and questions. This can be a somewhat resource-intensive process, "
                        "particularly if you have a very large database. When should beesbuddy automatically scan "
...
(def refingerprint
  ...
  {:name         "refingerprint"
   ...
   :description  (deferred-tru
                   (str "This enables beesbuddy to scan for additional field values during syncs allowing smarter "
...
(def default-options
  ...
  See the [plugin manifest reference](https://github.com/beesbuddy/beesbuddy/wiki/beesbuddy-Plugin-Manifest-Reference)
  for more details."
...
(def cloud-ip-address-info
  ...
  to being returned to the client, in [[beesbuddy.driver.util/connection-props-server->client]]."
  {:name   "cloud-ip-address-info"
   :type   :info
   :getter (fn []
             (when-let [ips (public-settings/cloud-gateway-ips)]
               (str (deferred-tru
                      (str "If your database is behind a firewall, you may need to allow connections from our beesbuddy "
                           "[Cloud IP addresses](https://www.beesbuddy.com/cloud/docs/ip-addresses-to-whitelist.html):"))
                    "\n"
                    (str/join " - " ips))))})
...
(defn class->base-type
  ...
  (condp #(isa? %2 %1) klass
    ...
    ;; discussion in the `beesbuddy.types` namespace.
...
(def ^:dynamic *start-of-week*
  "Used to override the [[beesbuddy.public-settings/start-of-week]] settings.
...
(mu/defn start-of-week->int :- [:int {:min 0, :max 6, :error/message "Start of week integer"}]
  "Returns the int value for the current [[beesbuddy.public-settings/start-of-week]] Setting value, which ranges from
...
(defn start-of-week-offset-for-day
  ...
  of week in the range 1..7 with [[beesbuddy.public-settings/start-of-week]] as 1."
  [start-of-week]
...
(mu/defn start-of-week-offset :- :int
  ...
  corresponding to [[driver/db-start-of-week]], so that `1` corresponds to [[beesbuddy.public-settings/start-of-week]] in
  results.

  e.g.

  If `:my-driver` returns [[driver/db-start-of-week]] as `:sunday` (1 is Sunday, 2 is Monday, and so forth),
  and [[beesbuddy.public-settings/start-of-week]] is `:monday` (the results should have 1 as Monday, 2 as Tuesday... 7 is
