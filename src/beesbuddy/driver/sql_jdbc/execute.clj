(ns beesbuddy.driver.sql-jdbc.execute
  ...
  `beesbuddy.driver.sql-jdbc.execute.old-impl`, which will be removed in a future release; implementations of methods
  for JDBC drivers that do not support `java.time` classes can be found in
  `beesbuddy.driver.sql-jdbc.execute.legacy-impl`. "
  (:require
   ...
   [beesbuddy.driver.sql-jdbc.connection :as sql-jdbc.conn]
   [beesbuddy.driver.sql-jdbc.execute.diagnostic :as sql-jdbc.execute.diagnostic]
   [beesbuddy.driver.sql-jdbc.execute.old-impl :as sql-jdbc.execute.old]
   [beesbuddy.driver.sql-jdbc.sync.interface :as sql-jdbc.sync.interface]
   [beesbuddy.lib.metadata :as lib.metadata]
   [beesbuddy.lib.schema.common :as lib.schema.common]
   [beesbuddy.lib.schema.expression.temporal :as lib.schema.expression.temporal]
   [beesbuddy.models.setting :refer [defsetting]]
   [beesbuddy.query-processor.error-type :as qp.error-type]
   [beesbuddy.query-processor.middleware.limit :as limit]
   [beesbuddy.query-processor.pipeline :as qp.pipeline]
   [beesbuddy.query-processor.reducible :as qp.reducible]
   [beesbuddy.query-processor.store :as qp.store]
   [beesbuddy.query-processor.timezone :as qp.timezone]
   [beesbuddy.query-processor.util :as qp.util]
   [beesbuddy.util :as u]
...
(defmulti do-with-connection-with-options
  ...
  * If `:write?` is NOT passed or otherwise falsey, make the connection read-only if possible; if it is truthy, make
  ...
  1. Specifying the session timezone based on the value of [[beesbuddy.driver/report-timezone]] as a JDBC connection
     parameter in the JDBC connection spec returned by [[beesbuddy.driver.sql-jdbc.connection/connection-details->spec]].
...
;; things in a `try-catch` and call `.close` (beesbuddy#40010)
...
;; TODO - since we're not running the queries in a transaction, does this make any difference at all? (beesbuddy#40012)
...
(mu/defn do-with-resolved-connection-data-source :- (lib.schema.common/instance-of-class DataSource)
  ...
  (if-not (u/id db-or-id-or-spec)
    ...
    (if-let [old-method-impl (get-method
      ...
      (do
        (log/warnf "%s is deprecated in beesbuddy 0.47.0. Implement %s instead."
...
(defn execute-reducible-query
  "Default impl of [[beesbuddy.driver/execute-reducible-query]] for sql-jdbc drivers."
      ...
      (with-open [stmt          (statement-or-prepared-statement driver conn sql params qp.pipeline/*canceled-chan*)
        ...
        (let [rsmeta           (.getMetaData rs)
          ...
          (try (respond results-metadata (reducible-rows driver rs rsmeta qp.pipeline/*canceled-chan*))
               ...
               ;; [[beesbuddy.query-processor.middleware.limit/limit-xform]] middleware, while statment is still
