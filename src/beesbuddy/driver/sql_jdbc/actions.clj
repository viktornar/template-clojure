(ns beesbuddy.driver.sql-jdbc.actions
  (:require
   ...
   [medley.core :as m]
   [beesbuddy.actions.core :as actions]
...
(defmulti maybe-parse-sql-error
  ...
  - type: the error type. Check [[beesbuddy.actions.error]] for the full list
...
(defmulti base-type->sql-type-map
  "Return a map of [[beesbuddy.types]] type to SQL string type name. Used for casting. Looks like we're just copypasting
  this from implementations of [[beesbuddy.test.data.sql/field-base-type->sql-type]] so go find that stuff if you need
...
(defmethod actions/perform-action!* [:sql-jdbc :row/delete]
  ...
  (let [raw-hsql    (mbql-query->raw-hsql driver query)
    ...
    (with-jdbc-transaction [conn database-id]
      ;; TODO -- this should probably be using [[beesbuddy.driver/execute-write-query!]]
