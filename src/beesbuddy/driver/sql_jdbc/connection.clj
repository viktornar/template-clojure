(ns beesbuddy.driver.sql-jdbc.connection
  ...
  (:require
   ...
   [beesbuddy.connection-pool :as connection-pool]
   [beesbuddy.db :as mdb]
   [beesbuddy.driver :as driver]
   [beesbuddy.lib.metadata :as lib.metadata]
   [beesbuddy.lib.metadata.jvm :as lib.metadata.jvm]
   [beesbuddy.models.interface :as mi]
...
When all connections are in use then beesbuddy will be slower to return results for queries, since it would have to wait for an available connection before processing the next query in the queue.
...
(setting/defsetting jdbc-data-warehouse-unreturned-connection-timeout-seconds
  ...
  timeout in [[beesbuddy.query-processor.context/query-timeout-ms]] by default."
  :visibility :internal
  :type       :integer
  :getter     (fn []
...
(defmethod data-warehouse-connection-pool-properties :default
  ...
  { ;; only fetch one new connection at a time, rather than batching fetches (default = 3 at a time). This is done in
   ...
   ;; request. IRL the beesbuddy server and data warehouse are likely to be located in closer geographical proximity to
