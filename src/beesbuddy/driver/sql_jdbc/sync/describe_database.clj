(ns beesbuddy.driver.sql-jdbc.sync.describe-database
  ...
  (:require
   ...
   [beesbuddy.driver.sync :as driver.s]
...
(defmethod sql-jdbc.sync.interface/filtered-syncable-schemas :sql-jdbc
  ...
  (eduction (remove (set (sql-jdbc.sync.interface/excluded-schemas driver)))
            ...
            (remove (fn [schema] (re-find #"^beesbuddy_cache.*" schema)))
...
(mu/defn describe-database
  "Default implementation of [[beesbuddy.driver/describe-database]] for SQL JDBC drivers. Uses JDBC DatabaseMetaData."
  [driver           :- :keyword
