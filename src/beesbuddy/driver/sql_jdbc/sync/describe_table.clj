(ns beesbuddy.driver.sql-jdbc.sync.describe-table
  ...
  (:require
   ...
   [medley.core :as m]
   [beesbuddy.db.metadata-queries :as metadata-queries]
   ...
   [beesbuddy.driver.sql-jdbc.sync.common :as sql-jdbc.sync.common]
   [beesbuddy.driver.sql-jdbc.sync.interface :as sql-jdbc.sync.interface]
   [beesbuddy.driver.sql.query-processor :as sql.qp]
   [beesbuddy.lib.schema.literal :as lib.schema.literal]
   [beesbuddy.models :refer [Field]]
   [beesbuddy.models.table :as table]
   [beesbuddy.util :as u]
   [beesbuddy.util.honey-sql-2 :as h2x]
   [beesbuddy.util.log :as log]
   [beesbuddy.util.malli.registry :as mr]
...
(defn- database-type->base-type-or-warn
  "Given a `database-type` (e.g. `VARCHAR`) return the mapped beesbuddy type (e.g. `:type/Text`)."
...
(defmulti describe-fields-sql
  "Returns a SQL query ([sql & params]) for use in the default JDBC implementation of [[beesbuddy.driver/describe-fields]],
 i.e. [[describe-fields]]."
  {:added    "0.49.1"
...
(defn describe-fields
  "Default implementation of [[beesbuddy.driver/describe-fields]] for JDBC drivers. Uses JDBC DatabaseMetaData."
...
(defn describe-table-fks
  "Default implementation of [[beesbuddy.driver/describe-table-fks]] for SQL JDBC drivers. Uses JDBC DatabaseMetaData."
...
(defmulti describe-fks-sql
 "Returns a SQL query ([sql & params]) for use in the default JDBC implementation of [[beesbuddy.driver/describe-fks]],
 i.e. [[describe-fks]]."
 {:added    "0.49.0"
  :arglists '([driver & {:keys [schema-names table-names]}])}
 driver/dispatch-on-initialized-driver
...
(defn describe-fks
  "Default implementation of [[beesbuddy.driver/describe-fks]] for JDBC drivers. Uses JDBC DatabaseMetaData."
  [driver db & {:keys [schema-names table-names] :as args}]
  (if (or (and schema-names (empty? schema-names))
          (and table-names (empty? table-names)))
    []
...
(defn describe-table-indexes
  "Default implementation of [[beesbuddy.driver/describe-table-indexes]] for SQL JDBC drivers. Uses JDBC DatabaseMetaData."
  [driver db table]
  (sql-jdbc.execute/do-with-connection-with-options
   driver
   db
