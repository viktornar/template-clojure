(ns beesbuddy.driver.h2
  (:require
   ...
   [beesbuddy.driver.h2.actions :as h2.actions]
   ...
   [beesbuddy.util.i18n :refer [deferred-tru tru]]
   [beesbuddy.util.log :as log]
   [beesbuddy.util.malli :as mu]
   [beesbuddy.util.ssh :as ssh])
  (:import
...
(defmethod driver/connection-properties :h2
  ...
  (->>
   [{:name         "db"
     ...
     :helper-text (deferred-tru "The local path relative to where beesbuddy is running from. Your string should not include the .mv.db extension.")
...
(defn- connection-string-set-safe-options
  "Add beesbuddy Security Settings™ to this `connection-string` (i.e. try to keep shady users from writing nasty SQL)."
  [connection-string]
  {:pre [(string? connection-string)]}
...
(defmethod driver/upload-type->database-type :h2
  [_driver upload-type]
  (case upload-type
    :beesbuddy.upload/varchar-255              [:varchar]
    :beesbuddy.upload/text                     [:varchar]
    :beesbuddy.upload/int                      [:bigint]
    :beesbuddy.upload/auto-incrementing-int-pk [:bigint :generated-always :as :identity]
    :beesbuddy.upload/float                    [(keyword "DOUBLE PRECISION")]
    :beesbuddy.upload/boolean                  [:boolean]
    :beesbuddy.upload/date                     [:date]
    :beesbuddy.upload/datetime                 [:timestamp]
    :beesbuddy.upload/offset-datetime          [:timestamp-with-time-zone]))
