(ns beesbuddy.driver.mysql
  ...
  (:require
   ...
   [beesbuddy.driver.mysql.actions :as mysql.actions]
   [beesbuddy.driver.mysql.ddl :as mysql.ddl]
   [beesbuddy.driver.sql-jdbc.common :as sql-jdbc.common]
   [beesbuddy.driver.sql-jdbc.connection :as sql-jdbc.conn]
   [beesbuddy.driver.sql-jdbc.execute :as sql-jdbc.execute]
   [beesbuddy.driver.sql-jdbc.sync :as sql-jdbc.sync]
   [beesbuddy.driver.sql.query-processor :as sql.qp]
   [beesbuddy.driver.sql.query-processor.util :as sql.qp.u]
   [beesbuddy.driver.sql.util :as sql.u]
   [beesbuddy.driver.sql.util.unprepare :as unprepare]
   [beesbuddy.lib.field :as lib.field]
...
(defmethod driver/database-supports? [:mysql :table-privileges]
  [_driver _feat _db]
  ;; Disabled completely due to errors when dealing with partial revokes (beesbuddy#38499)
  false
  #_(and (= driver :mysql) (not (mariadb? db))))

;;; +----------------------------------------------------------------------------------------------------------------+
;;; |                                             beesbuddy.driver impls                                              |
;;; +----------------------------------------------------------------------------------------------------------------+
       ...
       (when (unsupported-version? (.getMetaData conn))
         (log/warn
          (u/format-color :red
                          (str "\n\n********************************************************************************\n"
                               (format
                                "WARNING: beesbuddy only officially supports MySQL %s/MariaDB %s and above."
                                min-supported-mysql-version
                                min-supported-mariadb-version)
                               "\n"
                               "All beesbuddy features may not work properly when using an unsupported version."
                               "\n********************************************************************************\n"))))))))
...
;;; +----------------------------------------------------------------------------------------------------------------+
;;; |                                         beesbuddy.driver.sql-jdbc impls                                         |
;;; +----------------------------------------------------------------------------------------------------------------+
...
(def ^:private default-connection-args
  ...
  { ;; 0000-00-00 dates are valid in MySQL; convert these to `null` when they come back because they're illegal in Java
   ...
   ;; GZIP compress packets sent between beesbuddy server and MySQL/MariaDB database
   :useCompression       true})
...
(defn- parse-grant
  "Parses the contents of a row from the output of a `SHOW GRANTS` statement, to extract the data needed
   ...
   Privilege example:
   (parse-grant \"GRANT SELECT, INSERT, UPDATE, DELETE ON `test-data`.* TO 'beesbuddy'@'localhost'\")
   =>
   {:type            :privileges
   ...
   Role example:
   (parse-grant \"GRANT 'example_role_1'@'%','example_role_2'@'%' TO 'beesbuddy'@'localhost'\")
   =>
   {:type  :roles
