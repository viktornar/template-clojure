(ns beesbuddy.driver.ddl.interface
  (:require
...
(mu/defn schema-name :- ::lib.schema.common/non-blank-string
  ...
  multiple connections from multiple beesbuddys remain distinct. The UUID will have the first character of each section taken.

  (schema-name {:id 234} \"143dd8ce-e116-4c7f-8d6d-32e99eaefbbc\") ->  \"beesbuddy_cache_1e483_1\""
  ...
  (let [instance-string (apply str (map first (str/split site-uuid-string #"-")))]
    (format "beesbuddy_cache_%s_%s" instance-string id)))
...
(defmulti format-name
  ...
  from [[beesbuddy.test.data.sql/qualified-name-components]] -- so if you include the Database name there, it will get
