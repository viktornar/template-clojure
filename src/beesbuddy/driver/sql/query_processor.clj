(ns beesbuddy.driver.sql.query-processor
  "The Query Processor is responsible for translating the beesbuddy Query Language into HoneySQL SQL forms."
  (:require
   ...
   [beesbuddy.driver.common :as driver.common]
   [beesbuddy.driver.sql.query-processor.deprecated :as sql.qp.deprecated]
   [beesbuddy.legacy-mbql.schema :as mbql.s]
   [beesbuddy.legacy-mbql.util :as mbql.u]
   [beesbuddy.lib.convert :as lib.convert]
   [beesbuddy.lib.metadata :as lib.metadata]
   [beesbuddy.lib.query :as lib.query]
   [beesbuddy.lib.schema.common :as lib.schema.common]
   [beesbuddy.lib.util.match :as lib.util.match]
   [beesbuddy.query-processor.error-type :as qp.error-type]
   [beesbuddy.query-processor.middleware.annotate :as annotate]
   [beesbuddy.query-processor.middleware.wrap-value-literals :as qp.wrap-value-literals]
   [beesbuddy.query-processor.store :as qp.store]
   [beesbuddy.query-processor.util.add-alias-info :as add]
   [beesbuddy.query-processor.util.nest-query :as nest-query]
   [beesbuddy.query-processor.util.transformations.nest-breakouts :as qp.util.transformations.nest-breakouts]
   [beesbuddy.util :as u]
   [beesbuddy.util.honey-sql-2 :as h2x]
   [beesbuddy.util.i18n :refer [tru]]
   [beesbuddy.util.log :as log]
   [beesbuddy.util.malli :as mu]))
...
(defn make-nestable-sql
  ...
  https://github.com/beesbuddy/beesbuddy/pull/30677

  For the limitations see the [[beesbuddy.driver.sql.query-processor-test/make-nestable-sql-test]]"  [sql]
...
;; First week begins on 1st Jan, the 2nd week will begins on the 1st [[beesbuddy.public-settings/start-of-week]]
...
(defmulti add-interval-honeysql-form
  ...
  `unit` is one of the units listed in [[beesbuddy.util.date-2/add-units]].
...
(mu/defn adjust-day-of-week
  "Adjust day of week to respect the [[beesbuddy.public-settings/start-of-week]] Setting.
...
(defmulti quote-style
  ...
  [[beesbuddy.util.honey-sql-2]]; like `:ansi`, but uppercases the result). Check [[honey.sql/dialects]] for all
...
(defn- throw-double-compilation-error
  ...
  the past and some rely on this behavior (see ;;; [[beesbuddy.driver.bigquery-cloud-sdk.query-processor]]
  and [[beesbuddy.driver.snowflake]] for example). Maybe we come up with some better way to handle this -- e.g. maybe
  ...
  (if (some #(str/includes? (name %) "_") (keys field))
    (do
      (sql.qp.deprecated/log-deprecation-warning
       driver
       "beesbuddy.driver.sql.query-processor/cast-field-id-needed with a legacy (snake_cased) :model/Field"
       "0.48.0")
...
(defmethod ->honeysql [:sql :aggregation]
  ...
  (lib.util.match/match-one (nth (:aggregation *inner-query*) index)
    ...
    ;; by [[beesbuddy.query-processor.middleware.pre-alias-aggregations]]
    [ag-type & _]
...
(mu/defn field-clause->alias :- some?
  ...
  ([driver                                                :- :keyword
   ...
   (let [desired-alias (or desired-alias
                           ...
                           ;; from [[beesbuddy.query-processor.util.add-alias-info]]. We should probably disallow this
  ...
  ([driver field-clause _unique-name-fn]
   (sql.qp.deprecated/log-deprecation-warning
    driver
    "beesbuddy.driver.sql.query-processor/field-clause->alias with 3 args"
    "0.48.0")
...
(defn- apply-joins-honey-sql-2
  ...
  See [[beesbuddy.query-processor-test.explicit-joins-test/join-order-test]]."
...
(defmethod ->honeysql [:sql :model/Table]
  ...
  (sql.qp.deprecated/log-deprecation-warning
   driver
   "beesbuddy.driver.sql.query-processor/->honeysql for beesbuddy.models.table/Table or :model/Table"
   "0.48.0")
...
(mu/defn mbql->native :- [:map
  ...
  of [[driver/mbql->native]] (actual multimethod definition is in [[beesbuddy.driver.sql]]."
  [driver      :- :keyword
   outer-query :- :map]
