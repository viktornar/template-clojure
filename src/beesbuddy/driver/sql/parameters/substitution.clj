(ns beesbuddy.driver.sql.parameters.substitution
  ...
  (:require
   ...
   [beesbuddy.driver.common.parameters.dates :as params.dates]
   [beesbuddy.driver.common.parameters.operators :as params.ops]
   ...
   [beesbuddy.lib.schema.metadata :as lib.schema.metadata]
   [beesbuddy.lib.schema.parameter :as lib.schema.parameter]
   ...
   [beesbuddy.shared.util.time :as shared.ut]
   [beesbuddy.util :as u]
  ...
  (:import
   ...
   (java.util UUID)
   (beesbuddy.driver.common.parameters Date DateRange FieldFilter ReferencedCardQuery ReferencedQuerySnippet)))
...
(defmulti align-temporal-unit-with-param-type
  ...
  The resulting keyword will be used to call the corresponding `beesbuddy.driver.sql.query-processor/date`
...
(mu/defn ^:private field->clause :- mbql.s/field
  [driver     :- :keyword
   ...
   value]
  ;; The [[beesbuddy.query-processor.middleware.parameters/substitute-parameters]] QP middleware actually happens before
  ;; the [[beesbuddy.query-processor.middleware.resolve-fields/resolve-fields]] middleware that would normally fetch all
