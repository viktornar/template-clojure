(ns beesbuddy.plugins.lazy-loaded-driver
  ...
  See https://github.com/beesbuddy/beesbuddy/wiki/beesbuddy-Plugin-Manifest-Reference for all the options allowed for a
  plugin manifest."
  (:require
   [beesbuddy.driver :as driver]
   [beesbuddy.driver.common :as driver.common]
   [beesbuddy.plugins.init-steps :as init-steps]
   [beesbuddy.util :as u]
...
(defn register-lazy-loaded-driver!
  "Register a basic shell of a beesbuddy driver using the information from its beesbuddy plugin"
  ...
  (let [driver           (keyword driver-name)
    ...
    ;; finally, register the beesbuddy driver
