(ns beesbuddy.plugins.dependencies
  (:require
...
(defmethod dependency-satisfied? :default [_ {{plugin-name :name} :info} dep]
  (log/error
   (u/format-color :red "Plugin %s declares a dependency that beesbuddy does not understand: %s" plugin-name dep)
   "Refer to the plugin manifest reference for a complete list of valid plugin dependencies:"
   "https://github.com/beesbuddy/beesbuddy/wiki/beesbuddy-Plugin-Manifest-Reference")
  false)
...
(defn- warn-about-required-dependencies [plugin-name message]
  (log-once plugin-name
    (str (u/format-color 'red (trs "beesbuddy cannot initialize plugin {0} due to required dependencies." plugin-name))
         " "
         message)))
