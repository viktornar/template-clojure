(ns beesbuddy.plugins.initialize
  "Logic related to initializing plugins, i.e. running the `init` steps listed in the plugin manifest. This is done when
  beesbuddy launches as soon as all dependencies for that plugin are met; for plugins with unmet dependencies, it is
  ...
  (:require
   [beesbuddy.plugins.dependencies :as deps]
   [beesbuddy.plugins.init-steps :as init-steps]
   [beesbuddy.plugins.lazy-loaded-driver :as lazy-loaded-driver]
   [beesbuddy.util :as u]
