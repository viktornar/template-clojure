(ns beesbuddy.cmd
  "Functions for commands that can be ran from the command-line with the Clojure CLI or the beesbuddy JAR. These are ran
  as follows:

    <beesbuddy> <command> <options>
  ...
  for example, running the `migrate` command and passing it `force` can be done using one of the following ways:
    ...
    java -jar beesbuddy.jar migrate force
...
(defn- cmd->var
  ...
  [command-name]
  (ns-resolve 'beesbuddy.cmd (symbol command-name)))
...
(defn- call-enterprise
  ...
  (let [f (try
            ...
            (catch Throwable e
              (throw (ex-info (trs "The ''{0}'' command is only available in beesbuddy Enterprise Edition." (name symb))
                              {:command symb}
                              e))))]
    (apply f args)))
...
(defn ^:command migrate
  ...
  [direction]
  (classloader/require 'beesbuddy.cmd.migrate)
  ((resolve 'beesbuddy.cmd.migrate/migrate!) direction))
...
(defn ^:command load-from-h2
  ...
  ([h2-connection-string]
   (classloader/require 'beesbuddy.cmd.load-from-h2)
   ((resolve 'beesbuddy.cmd.load-from-h2/load-from-h2!) h2-connection-string)))
  ...
  {:doc "Transfer data from existing database to newly created H2 DB with specified filename.
   ...
   :arg-spec [["-k" "--keep-existing" "Do not delete target H2 file if it exists."
              ...
              ["-p" "--dump-plaintext" "Do not encrypt dumped contents."
               :id :dump-plaintext?]]}
  [h2-filename & opts]
  (classloader/require 'beesbuddy.cmd.dump-to-h2)
  (try
    (let [options (get-parsed-options #'dump-to-h2 opts)]
      ((resolve 'beesbuddy.cmd.dump-to-h2/dump-to-h2!) h2-filename options)
...
(defn ^:command profile
  "Start beesbuddy the usual way and exit. Useful for profiling beesbuddy launch time."
  []
  ;; override env var that would normally make Jetty block forever
  (alter-var-root #'env/env assoc :bb-jetty-join "false")
  (u/profile "start-normally" ((resolve 'beesbuddy.core/start-normally))))

(defn ^:command reset-password
  "Reset the password for a user with `email-address`."
  [email-address]
  (classloader/require 'beesbuddy.cmd.reset-password)
  ((resolve 'beesbuddy.cmd.reset-password/reset-password!) email-address))

(defn ^:command help
  "Show this help message listing valid beesbuddy commands."
  ([command-name]
  ...
  ([]
   ...
   (doseq [[symb varr] (sort (ns-interns 'beesbuddy.cmd))
           :when       (:command (meta varr))]
     (help symb)
     (println))
   (println "\nSome other commands you might find useful:\n")
   (println "java -cp beesbuddy.jar org.h2.tools.Shell -url jdbc:h2:/path/to/beesbuddy.db")
   (println "\tOpen an SQL shell for the beesbuddy H2 DB")))

(defn ^:command version
  "Print version information about beesbuddy and the current system."
  []
  (println "beesbuddy version:" config/mb-version-info)
  (println "\nOS:"
...
(defn ^:command api-documentation
  ...
  []
  (classloader/require 'beesbuddy.cmd.endpoint-dox)
  ((resolve 'beesbuddy.cmd.endpoint-dox/generate-dox!)))

(defn ^:command environment-variables-documentation
  "Generates a markdown file containing documentation for environment variables relevant to configuring beesbuddy.
  The command only includes environment variables registered as defsettings.
  For a full list of environment variables, see https://www.beesbuddy.com/docs/latest/configuring-beesbuddy/environment-variables."
  []
  (classloader/require 'beesbuddy.cmd.env-var-dox)
  ((resolve 'beesbuddy.cmd.env-var-dox/generate-dox!)))
...
(defn ^:command driver-methods
  ...
  ([]
   (classloader/require 'beesbuddy.cmd.driver-methods)
   ((resolve 'beesbuddy.cmd.driver-methods/print-available-multimethods) false))
  ([_docs]
   (classloader/require 'beesbuddy.cmd.driver-methods)
   ((resolve 'beesbuddy.cmd.driver-methods/print-available-multimethods) true)))

(defn ^:command import
  {:doc "Load serialized beesbuddy instance as created by the [[export]] command from directory `path`."}
  [path & options]
  (call-enterprise 'beesbuddy-enterprise.serialization.cmd/v2-load! path (get-parsed-options #'import options)))

(defn ^:command export
  {:doc "Serialize beesbuddy instance into directory at `path`."
  ...
  [path & options]
  (call-enterprise 'beesbuddy-enterprise.serialization.cmd/v2-dump! path (get-parsed-options #'export options)))

(defn ^:command seed-entity-ids
  "Add entity IDs for instances of serializable models that don't already have them."
  []
  (when-not (call-enterprise 'beesbuddy-enterprise.serialization.cmd/seed-entity-ids!)
...
(defn ^:command drop-entity-ids
  ...
  to v2 (x.47+)."
  []
  (when-not (call-enterprise 'beesbuddy-enterprise.serialization.cmd/drop-entity-ids!)
    (throw (Exception. "Error encountered while dropping entity IDs"))))

(defn ^:command rotate-encryption-key
  "Rotate the encryption key of a beesbuddy database. The MB_ENCRYPTION_SECRET_KEY environment variable has to be set to
  the current key, and the parameter `new-key` has to be the new key. `new-key` has to be at least 16 chars."
  [new-key]
  (classloader/require 'beesbuddy.cmd.rotate-encryption-key)
  (try
    ((resolve 'beesbuddy.cmd.rotate-encryption-key/rotate-encryption-key!) new-key)
  ...
  (let [varr (cmd->var command-name)
    ...
    (cond
      ...
      [(str "Unrecognized command: '" command-name "'")
       (str "Valid commands: " (str/join ", " (map key (filter (comp :command meta val) (ns-interns 'beesbuddy.cmd)))))]
...
(defn run-cmd
  "Run `cmd` with `args`. This is a function above. e.g. `clojure -M:run beesbuddy migrate force` becomes
  `(migrate \"force\")`."
  [command-name args]
