(ns beesbuddy.server
  "Code related to configuring, starting, and stopping the beesbuddy Jetty web server."
  (:require
   [clojure.core :as core]
   [clojure.string :as str]
   [medley.core :as m]
   [beesbuddy.config :as config]
   [beesbuddy.server.protocols :as server.protocols]
   [beesbuddy.util :as u]
...
(defn- jetty-ssl-config []
  (m/filter-vals
   some?
   {:ssl-port        (config/config-int :bb-jetty-ssl-port)
    :keystore        (config/config-str :bb-jetty-ssl-keystore)
    :key-password    (config/config-str :bb-jetty-ssl-keystore-password)
    :truststore      (config/config-str :bb-jetty-ssl-truststore)
    :trust-password  (config/config-str :bb-jetty-ssl-truststore-password)
    :client-auth     (when (config/config-bool :bb-jetty-ssl-client-auth)
                       :need)
    :sni-host-check? (when (config/config-str :bb-jetty-skip-sni)
                       false)}))

(defn- jetty-config []
  (cond-> (m/filter-vals
           some?
           {:port          (config/config-int :bb-jetty-port)
            :host          (config/config-str :bb-jetty-host)
            :max-threads   (config/config-int :bb-jetty-maxthreads)
            :min-threads   (config/config-int :bb-jetty-minthreads)
            :max-queued    (config/config-int :bb-jetty-maxqueued)
            :max-idle-time (config/config-int :bb-jetty-maxidletime)})
    (config/config-int :bb-jetty-request-header-size) (assoc :request-header-size (config/config-int
                                                                                   :bb-jetty-request-header-size))
    (config/config-str :bb-jetty-daemon) (assoc :daemon? (config/config-bool :bb-jetty-daemon))
    (config/config-str :bb-jetty-ssl)    (-> (assoc :ssl? true)
...
(defn create-server
  "Create a new async Jetty server with `handler` and `options`. Handy for creating the real beesbuddy web server, and
  ...
  (let [timeout       (config/config-int :bb-jetty-async-response-timeout)
...
(defn start-web-server!
  ...
  running server.
    ...
    (start-web-server! #'beesbuddy.server.handler/app)"
  [handler]
  (when-not (instance)
