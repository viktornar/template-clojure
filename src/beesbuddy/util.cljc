(ns beesbuddy.util
  ...
  (:require
   #?@(:clj  ([clojure.math.numeric-tower :as math]
              ...
              [beesbuddy.util.jvm :as u.jvm]
              [beesbuddy.util.string :as u.str]
              [potemkin :as p]
   ...
   [medley.core :as m]
   [beesbuddy.shared.util.i18n :refer [tru] :as i18n]
   [beesbuddy.shared.util.namespaces :as u.ns]
   [beesbuddy.util.format :as u.format]
   [beesbuddy.util.log :as log]
   [beesbuddy.util.memoize :as memoize]
...
(defn slugify
  ...
  replaced with underscores in order to support languages that don't use the Latin alphabet; see beesbuddy#3818).
...
(defn email-in-domain?
  "Is `email-address` in `domain`?
    ...
    (email-in-domain? \"cam@toucan.farm\" \"beesbuddy.com\") ; -> false"
