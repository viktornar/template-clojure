(ns beesbuddy.util.jvm
  "JVM-specific utilities and helpers.
  You don't want to import this namespace directly - these functions are re-exported by [[beesbuddy.util]]."
  (:require
   [clojure.java.classpath :as classpath]
   [clojure.string :as str]
   [clojure.tools.namespace.find :as ns.find]
   [beesbuddy.shared.util.i18n :refer [tru]]
   [beesbuddy.util.format :as u.format]
...
(defprotocol ^:private IFilteredStacktrace
  (filtered-stacktrace [this]
    "Get the stack trace associated with E and return it as a vector with non-beesbuddy frames after the last Metabase
    frame filtered out."))
...
(extend (Class/forName "[Ljava.lang.StackTraceElement;")
  ...
  {:filtered-stacktrace
   (fn [this]
     ...
     ;; that
     (let [[frames-after-last-mb other-frames]     (split-with #(not (str/includes? % "beesbuddy"))
                                                               (seq this))
           [last-mb-frame & frames-before-last-mb] (for [frame other-frames
                                                         :when (str/includes? frame "beesbuddy")]
                                                     (str/replace frame #"^beesbuddy\." ""))]
       (vec
        (concat
...
(defn do-with-auto-retries
  "Execute `f`, a function that takes no arguments, and return the results.
   ...
   consider using the `beesbuddy.util.retry` namespace."
  {:style/indent 1}
  [num-retries f]
  (if (<= num-retries 0)
    (f)
...
(defmacro auto-retry
  ...
  You can disable auto-retries for a specific ExceptionInfo by including `{:beesbuddy.util.jvm/no-auto-retry? true}` in
...
;;; TODO -- this is only used [[beesbuddy.analytics.snowplow-test]] these days
(defn decode-base64
