(ns beesbuddy.util.malli.fn
  ...
  (:require
   ...
   [beesbuddy.shared.util.i18n :as i18n]
   [beesbuddy.util.log :as log]
   [beesbuddy.util.malli.humanize :as mu.humanize]
...
(defn fn-schema
  "Implementation for [[fn]] and [[beesbuddy.util.malli.defn/defn]]. Given an unparsed parametered fn tail, extract the
...
(defn deparameterized-fn-form
  "Impl for [[beesbuddy.util.malli.fn/fn]] and [[beesbuddy.util.malli.defn/defn]]. Given a parsed `fn` tail (as parsed
...
(def ^:dynamic *enforce*
  ...
  use [[beesbuddy.util.malli/disable-enforcement]] to bind this only in Clojure code."
  true)
...
(defn validate-input
  "Impl for [[beesbuddy.util.malli.fn/fn]]; validates an input argument with `value` against `schema` using a cached
...
(defn validate-output
  "Impl for [[beesbuddy.util.malli.fn/fn]]; validates function output `value` against `schema` using a cached explainer
...
(defmacro fn
  ...
  The map arg here is additional error context; for something like [[beesbuddy.util.malli/defn]], it will be something
  like

    {:fn-name 'beesbuddy.lib.field/resolve-field-id}

  for [[beesbuddy.util.malli/defmethod]] it will be something like
