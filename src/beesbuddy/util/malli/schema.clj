(ns beesbuddy.util.malli.schema
  ...
  (:require
   ...
   [malli.core :as mc]
   [beesbuddy.legacy-mbql.normalize :as mbql.normalize]
   [beesbuddy.legacy-mbql.schema :as mbql.s]
   [beesbuddy.lib.schema.common :as lib.schema.common]
   [beesbuddy.models.dispatch :as models.dispatch]
   [beesbuddy.util :as u]
   [beesbuddy.util.date-2 :as u.date]
   [beesbuddy.util.i18n :as i18n :refer [deferred-tru]]
   [beesbuddy.util.malli :as mu]
   [beesbuddy.util.password :as u.password]))
...
(def BooleanValue
  ...
  Used by [[beesbuddy.api.common/defendpoint]] to coerce the value for this schema to a boolean.
...
(def Parameter
  ...
  We're not using [beesbuddy.legacy-mbql.schema/Parameter] here because this Parameter is meant to be used for
