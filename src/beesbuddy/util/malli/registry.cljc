(ns beesbuddy.util.malli.registry
  ...
  #?(:cljs (:require-macros [beesbuddy.util.malli.registry])))

(defonce ^:private cache (atom {}))

(defn cached
  "Get a cached value for `k` + `schema`. Cache is cleared whenever a schema is (re)defined
  with [[beesbuddy.util.malli.registry/def]]. If value doesn't exist, `value-thunk` is used to calculate (and cache)
  it.
...
#?(:clj
   (defmacro def
     ...
     ([type docstring schema]
      `(beesbuddy.util.malli.registry/def ~type
