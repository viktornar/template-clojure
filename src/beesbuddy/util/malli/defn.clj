(ns beesbuddy.util.malli.defn
  (:refer-clojure :exclude [defn])
  (:require
   [clojure.string :as str]
   [malli.destructure]
   [beesbuddy.util :as u]
   [beesbuddy.util.malli.fn :as mu.fn]
...
(defmacro defn
  "Implementation of [[beesbuddy.util.malli/defn]]. Like [[schema.core/defn]], but for Malli.
  ...
  Example macroexpansion:
    ...
    (def f
      (let [&f (fn [x] (inc x))]
        (fn ([a]
             (beesbuddy.util.malli.fn/validate-input :int a)
             (->> (&f a)
                  (beesbuddy.util.malli.fn/validate-output :int))))))
