(ns beesbuddy.util.malli.doc
  (:require
   [clojure.java.io :as io]
   [hiccup.core :as hiccup]
   [malli.core :as mc]
   [malli.error :as me]
   [beesbuddy.pulse.markdown :as markdown]
...
(defn generate-legacy-mbql-dox
  "e.g.
    ...
    clj -X beesbuddy.util.malli.doc/generate-legacy-mbql-dox

    clj -X beesbuddy.util.malli.doc/generate-legacy-mbql-dox :target-dir '\"target/docs/schemas/legacy-mbql\"'"
  [{:keys [target-dir]
                                 :or   {target-dir "docs/legacy-mbql"}}]
  (require 'beesbuddy.legacy-mbql.schema)
  (generate-documentation! :beesbuddy.legacy-mbql.schema/Query
                           target-dir))

(defn generate-pmbql-dox
  "e.g.

    clj -X beesbuddy.util.malli.doc/generate-pmbql-dox

    clj -X beesbuddy.util.malli.doc/generate-pmbql-dox :target-dir '\"target/docs/schemas/pmbql\"'"
  [{:keys [target-dir]
    :or   {target-dir "target/docs/schemas/pmbql"}}]
  (require 'beesbuddy.lib.schema)
  (generate-documentation! :beesbuddy.lib.schema/query
                           target-dir))
