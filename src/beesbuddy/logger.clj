(ns beesbuddy.logger
  "Configures the logger system for Beesuddy Sets up an in-memory logger in a ring buffer for showing in the UI. Other
  logging options are set in [[beesbuddy.bootstrap]]: the context locator for log4j2 and ensuring log4j2 is the logger
...
(defn- beesbuddy-appender ^Appender []
  (let [^org.apache.logging.log4j.core.Filter filter                   nil
    ...
    (proxy [org.apache.logging.log4j.core.appender.AbstractAppender]
           ["beesbuddy-appender" filter layout false properties]
      (append [event]
...
(when-not *compile-files*
  (when-not @has-added-appender?
    ...
    (let [appender (beesbuddy-appender)
