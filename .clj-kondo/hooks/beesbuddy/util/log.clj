  ...
  (walk/postwalk
   (fn [x]
     (when-let [qualified-symbol (hooks.common/node->qualified-symbol x)]
       (when (and (= (namespace qualified-symbol) "beesbuddy.util.i18n")
         ...
         (api/reg-finding! (assoc (meta x)
                                  ...
                                  :type :beesbuddy/validate-logging)))))
   node))
...
(comment
  ...
  (infof
   {:node (api/parse-string
           (str '(log/warnf (beesbuddy.util.i18n/tru "Migration lock was acquired after {0} retries." 1))))})

  (info
   {:node (api/parse-string
           (str '(log/warn (beesbuddy.util.i18n/trs "Migration lock was acquired after {0} retries." 1))))}))
