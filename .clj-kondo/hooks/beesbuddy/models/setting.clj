(ns hooks.beesbuddy.models.setting
  (:require
    ...
    (when (nil? (second (drop-while (comp not #{[:k :export?]} first) options-list)))
      (when-not (contains? ignored-implicit-export? (:value setting-name))
        (hooks/reg-finding! (assoc (meta node)
                                   ...
                                   :type :beesbuddy/defsetting-must-specify-export))))
...
(defn defsetting
  "Rewrite a [[beesbuddy.models.defsetting]] form like
...
(defn define-multi-setting
  "Rewrite a [[beesbuddy.models.define-multi-setting]] form like
  ...
  (defn x []
    (defsetting*
      '(defsetting active-users-count
         ...
         :default    0
         :getter     (fn []
                       (if-not ((requiring-resolve 'beesbuddy.db/db-is-set-up?))
                         0
