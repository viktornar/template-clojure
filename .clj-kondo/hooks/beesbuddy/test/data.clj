...
(defn- global-dataset-symbols
  "Dataset definitions defined in [[beesbuddy.test.data.dataset-definitions]]. This is only populated if clj-kondo has
  ...
  []
  (not-empty (set (keys (:clj (hooks/ns-analysis 'beesbuddy.test.data.dataset-definitions))))))

(defn- dataset-type
  "`dataset` can be one of:

  - a qualified symbol
  - an unqualified symbol referring to a a var in [[beesbuddy.test.data.dataset-definitions]]
  ...
  (let [[table query] (if (= (count args) 1)
    ...
    (when-not ((some-fn symbol? nil?) (hooks/sexpr table))
      (hooks/reg-finding! (assoc (meta table)
                                 ...
                                 :type :beesbuddy/mbql-query-first-arg)))
